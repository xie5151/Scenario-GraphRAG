...
def is_point_collide_cylinder(px, py, cyl):
    ...
    # 安全距离 = 圆柱半径 + 机器人半径 + 缓冲
    safe = cyl["radius"] + ROBOT_RADIUS + 0.15
    return dist < safe
...
def parse_path(raw_path):
    ...
    rospy.loginfo(f"解析到路径点数量：{len(parsed)}")
    return parsed
...
if __name__ == "__main__":
    try:
        rospy.init_node("turtlebot3_square_wall_exp", anonymous=True)
