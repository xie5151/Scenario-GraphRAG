...
START_POINT = (-0.5,-2.0,0.0)
END_POINT = (0.5,2.0,90.0)  
...
def get_llm_path():
    ...
    try:
        with open("/home/admin/graphrag_dev7_bak/obstacles_manual.txt", "r", encoding="utf-8") as f:
            obstacle_info = f.read()
    except Exception as e:
        rospy.logerr(f"读取障碍文件失败：{e}")
        return None
...
任务：规划从起点A(-0.5,-2.0,0.0)到终点B(0.5,2.0,90.0)的平滑避障路径
核心要求：
